//
//  SDKSynchronizerTransactionSubmissionTests.swift
...
final class SDKSynchronizerTransactionSubmissionTests: ZcashTestCase {
    ...
    func testCreateWithoutSubmittingReturnsRawAndManualSubmitUsesProvidedEndpoint() async throws {
        ...
        let transactions = try await synchronizer.createProposedTransactionsWithoutSubmitting(
            proposal: proposal,
        ...
        XCTAssertTrue(service.recordedTransactions().isEmpty, "Transactions should not be broadcast during creation.")

        await fulfillment(of: [foundTransactionsExpectation], timeout: 1.0)

        try await synchronizer.submitTransaction(rawTransaction, to: service.endpoint)
    ...
    func testSubmitTransactionThrowsSubmitErrorWhenEndpointRejectsTransaction() async throws {
        try await assertSubmitTransactionThrowsSubmitError(errorCode: -25, errorMessage: "rejected")
    }

    func testSubmitTransactionThrowsSubmitErrorWhenEndpointReturnsPositiveErrorCode() async throws {
        try await assertSubmitTransactionThrowsSubmitError(errorCode: 1, errorMessage: "already in mempool")
    }
    ...
    private func assertSubmitTransactionThrowsSubmitError(errorCode: Int32, errorMessage: String) async throws {
        ...
        do {
            try await synchronizer.submitTransaction(rawTransaction, to: service.endpoint)
            XCTFail("submitTransaction should throw when the server rejects the transaction.")
