...
pub unsafe extern "C" fn zcashlc_voting_validate_pir_proof(
    ...
    let res = catch_panic(|| {
        let root_bytes: [u8; 32] = unsafe { bytes_from_ptr(root, 32) }
            .try_into()
            .map_err(|_| anyhow!("root must be exactly 32 bytes"))?;
        let nf_bounds_bytes = unsafe { bytes_from_ptr(nf_bounds, 96) };
        let path_bytes = unsafe { bytes_from_ptr(path, 928) };
        let nullifier_bytes: [u8; 32] = unsafe { bytes_from_ptr(nullifier, 32) }
            .try_into()
            .map_err(|_| anyhow!("nullifier must be exactly 32 bytes"))?;
        let expected_root_bytes: [u8; 32] = unsafe { bytes_from_ptr(expected_root, 32) }
            .try_into()
...
mod tests {
    ...
    // https://github.com/valargroup/zcash_voting/blob/zcash_voting-v0.4.1/zcash_voting/src/zkp1.rs#L573-L708
