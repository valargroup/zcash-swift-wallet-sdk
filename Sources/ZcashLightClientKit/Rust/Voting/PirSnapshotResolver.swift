...
public enum PirSnapshotResolverError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .noMatchingEndpoint(let expected, let details):
            let summary = details.map { $0.shortDescription }.joined(separator: "; ")
            return "No PIR server matches the round's expected snapshot height \(expected). " +
                "Voting cannot proceed until a PIR server reports the matching snapshot. [\(summary)]"
        }
    }
}
...
public struct PirSnapshotProbeOutcome: Equatable, Sendable {
    ...
    public var shortDescription: String {
        switch status {
        case .matching(let h):
            return "\(url): matching@\(h)"
        case .mismatched(let h):
            return "\(url): mismatched@\(h)"
        case .missingHeight:
