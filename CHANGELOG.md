...
- `zcash_voting` dependency foundation: SDK Rust crate now depends on `zcash_voting 0.4` (`default-features = false`) and exposes a single pure-function FFI symbol, `zcashlc_voting_compute_share_nullifier`, used to prove end-to-end linkage against the voting crate. No Swift API surface is added in this step; wallet-to-voting plumbing, the broader FFI surface, and Swift wrappers will follow in subsequent PRs. Tracks #1661.
